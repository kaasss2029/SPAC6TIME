...
type MapArgs<T extends string[]> = {
    [K in keyof T]: T[K] extends keyof EmscriptenTypeMap ? EmscriptenTypeMap[T[K]] : never;
};
export interface WasmModuleMultiThread extends WasmModuleBase {
    _compute(threadsCount: number, runDataPointer: number): number;
    cwrap<Ret extends keyof EmscriptenTypeMap, const Args extends (keyof EmscriptenTypeMap)[], Async extends boolean = false>(ident: string, returnType: Ret, argTypes?: Args, options?: {
        async: Async;
    }): (...args: MapArgs<Args>) => Async extends true ? Promise<EmscriptenTypeMap[Ret]> : EmscriptenTypeMap[Ret];
}
export {};
