...
import type { MultiThreadRuntime, WasmRuntime } from './runtimes/wasm-runtime.js';
 ...
 */
type AnyCalculator = Calculator<string, number, TupleOf<string, number>, TypedArray | Record<string, TypedArray>, unknown, Record<string, unknown>>;
export type CalculatorsToFormattedOutput<Calculators extends readonly AnyCalculator[]> = {
    [K in Calculators[number]['name']]: ReturnType<Extract<Calculators[number], {
        name: K;
    }>['getFormattedOutput']>;
};
export type CalculatorsToRawOutput<Calculators extends readonly AnyCalculator[]> = {
    [K in Calculators[number]['name']]: ReturnType<Extract<Calculators[number], {
        name: K;
    }>['getRawOutput']>;
};
type RunParametersOf<C> = C extends Calculator<string, number, TupleOf<string, number>, TypedArray | Record<string, TypedArray>, unknown, infer RunParameters> ? RunParameters : never;
type NameOf<C> = C extends {
...
type RequiredRunParameters<Calculators extends readonly AnyCalculator[]> = {
    [C in Calculators[number] as HasRequiredRunParams<RunParametersOf<C>> extends true ? NameOf<C> : never]: RunParametersOf<C>;
};
/**
...
type OptionalRunParameters<Calculators extends readonly AnyCalculator[]> = {
    [C in Calculators[number] as HasRequiredRunParams<RunParametersOf<C>> extends true ? never : HasAnyRunParams<RunParametersOf<C>> extends true ? NameOf<C> : never]?: RunParametersOf<C>;
};
/**
 ...
 */
type BulkPropagatorRunArgs<Calculators extends readonly AnyCalculator[]> = RequiredRunParameters<Calculators> extends infer Required ? OptionalRunParameters<Calculators> extends infer Optional ? EmptyObject extends Required ? [DeclaredKeys<Optional>] extends [never] ? [runParameters?: NoRunParameters] : [runParameters?: Optional] : [runParameters: Required & Optional] : never : never;
/**
...
export declare class BulkPropagator<const Calculators extends readonly Calculator<string, number, TupleOf<string, number>, TypedArray | Record<string, TypedArray>, unknown>[], Runtime extends WasmRuntime> implements Disposable {
     ...
     */
    run(...[runParameters]: BulkPropagatorRunArgs<Calculators>): Runtime extends MultiThreadRuntime ? Promise<void> : undefined;
    /**
